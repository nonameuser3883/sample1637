...
export const extractAllColorVariables: FullDSExtractor = async () => {
  const collections = await figma.variables.getLocalVariableCollectionsAsync();
  const vars = await figma.variables.getLocalVariablesAsync('COLOR');

  const md = new MdBuilder().h1('Color Variables');

  if (vars.length === 0) {
    md.p('No local color variables found.');
    return { markdown: md.toString(), warnings: ['Нет локальных Color Variables.'] };
  }

  const warnings: string[] = [
    `Exported ${vars.length} color variables across ${collections.length} collection(s).`
  ];
    ...
    for (const [groupName, groupVars] of groups) {
      ...
      for (const v of groupVars) {
        ...
        for (const mode of collection.modes) {
          ...
          const str = await variableValueToString(raw, 'COLOR');
